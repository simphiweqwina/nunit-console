...
Task("UpdateAssemblyInfo")
    ...
    .Does<BuildParameters>((parms) =>
    {
        PatchAssemblyInfo("src/NUnitConsole/ConsoleVersion.cs", parms.PackageVersion, parms.PackageVersion);
        PatchAssemblyInfo("src/NUnitEngine/EngineApiVersion.cs", parms.PackageVersion, assemblyVersion: null);
        PatchAssemblyInfo("src/NUnitEngine/EngineVersion.cs", parms.PackageVersion, parms.PackageVersion);
    });
...
.Does<BuildParameters>((parms) =>
{
    MSBuild(parms.MsiDirectory + "nunit/nunit.wixproj", new MSBuildSettings()
        ...
        .WithProperty("Version", parms.PackageVersion) // Both of these should be just version
        .WithProperty("DisplayVersion", parms.PackageVersion)
